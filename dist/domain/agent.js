...
export function runAgentCycle(config, currentRun, currentCandidate, event) {
...
function observe(run, event) {
    ...
    if (event.type === "silence")
        return { type: "silence", text: event.text, summary: `silence after ${run.followups} follow-up(s)` };
    return { type: "candidate-reply", text: event.text, summary: `candidate replied: ${event.text}` };
}
function inferCandidate(run, observation) {
    ...
    if (!signals.length)
        signals.push("unclear reply");
    return { state: candidateState, objection, action: "Infer", sentiment, intent, signals, learned };
}
function chooseAction(policy, run, candidate, observation) {
    if (observation.type === "start")
        return decision("Start outreach", "first-touch", 12, run.followups, policy.actions.start || "Send first touch.");
    if (observation.type === "silence") {
        const nextFollowups = run.followups + 1;
        if (nextFollowups > policy.maxFollowups)
            return decision("Stop sequence", "closed", 0, nextFollowups, policy.actions.exit || "Stop.");
    ...
    if (candidate.intent === "negative")
        return decision("Exit", "closed", 0, run.followups, policy.actions.exit || "Exit.");
    if (candidate.intent === "positive")
        return decision("Advance to scheduling", "scheduling", 85, run.followups, policy.actions.advance || "Advance.");
    if (candidate.intent === "warm-timing" || candidate.intent === "passive")
        return decision("Lower commitment", "nurture", 46, run.followups, policy.actions.lower || "Lower commitment.");
    if (candidate.intent === "skeptical")
        return decision("Prove differentiation", "proof", 54, run.followups, policy.actions.prove || "Prove.");
    if (candidate.intent === "evaluating")
        return decision("Clarify scope", "qualification", 66, run.followups, policy.actions.clarify || "Clarify.");
...
function composeMessage(config, decision) {
